...
PyObject*
buffer_to_trimmed_string(buffer_t* buf, const char* end)
{
    PyObject* val = NULL;
    char* start;
    size_t used;
    ...
    // Trim whitespace
    while(IS_LOWER(*start) && used > 0)
    {
        start++;
        used--;
    }
    
    while(IS_LOWER(*(start+used-1)) && used > 0)
    {
        used--;
    }

    val = PyString_FromStringAndSize(start, used);
    if(val == NULL) PyErr_NoMemory();
